...
namespace PcShopIW5_DAL.Repositories
{
        ...
        where TEntity : class, IEntity, new()
    {
        ...
        private void SynchronizeCollections(TEntity entity)
        {
            ...
            {
                var updatedCollection = ((collectionSelector.GetValue(entity) 
                    as ICollection<IEntity>) ?? new List<IEntity>()).ToArray();
                var collectionInDb = ((collectionSelector.GetValue(entityInDb) 
                    as ICollection<IEntity>) ?? new List<IEntity>()).ToArray();
